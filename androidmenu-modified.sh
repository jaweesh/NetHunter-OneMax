...
LOCALGIT=1 # Toolchain and kernel (if set to 0 it will need fast internet because it will download the toolchaing and kernel)
FROZENKERNEL=0

#########  Devices  ##########
# Build scripts for each kernel is located under devices/devicename
source devices/one-max
...
d_clear
echo ""
echo -e "\e[31m ---- HTC One Max --------------------------------------------------------\e[0m" 
echo "  [1] Build for HTC One Max (Android 4.4+)"
echo ""
...
read -p "Choice: " onemax_menuchoice

case $onemax_menuchoice in

1) d_clear; f_rootfs ; f_flashzip ; f_onemax_stock_kernel ; f_zip_save ; f_zip_kernel_save ; f_rom_build ;;
2) d_clear; f_onemax_stock_kernel ; f_zip_kernel_save ;;
0) d_clear; f_interface ;;
*) echo "Incorrect choice..." ;
esac
  ...
  kernel)
    case $2 in
      ...
      onemax)
        ...
        f_zip_kernel_save
        cd ${basedir}
        mkdir -p $exportdir/Kernels/OneMax
        mv kernel-kali-$VERSION.zip $exportdir/Kernels/OneMax/Kernel-$device-$VERSION.zip
        mv kernel-kali-$VERSION.sha1sum $exportdir/Kernels/OneMax/Kernel-$device-$VERSION.zip.sha1sum
        rm -rf ${basedir}
        exit;;
